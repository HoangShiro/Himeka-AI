...
async def h_check():
    ...
    if await ai_status.day_time:
        if vn_time.hour == 7:
            if await ai_status.sleeping:
            ...
            if ready:
                await voice_rcn()
                if await ai_status.st_log:
                    print(f"{ai_name}'s tablet: đã nhắc {ai_name} thức dậy.")
                if await ai_status.chat_log:
                    print(f"{ai_name}: {answ}")

    # Tới giờ nghỉ trưa
    if await ai_status.non_time:
        if vn_time.hour == 12:
    ...
    if vn_time.hour == 1 or vn_time.hour == 5:
        if not await ai_status.sleeping:
            await ai_status.set('sleeping', True)
        if not await ai_status.day_time:
...
async def m_check():
    ...
    if await ai_status.sleep_cd > 0:
        await ai_status.update('sleep_cd', -1)
    elif await ai_status.sleep_cd == 0:
        await ai_status.set('sleeping', True)
